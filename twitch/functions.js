...
window.addEventListener('onEventReceived', function (obj) {
    ...
    if (listener === 'message') {
      //Agregar esto al if para acceso a mods = event.data.tags.badges.includes('moderator')
      if(event.data.tags.badges.includes('broadcaster')) {
        if(event.renderedText === manaOnCommand){
        	setManaStatus(true);
      	}
        if(event.renderedText === manaOffCommand){
          	setManaStatus(false);
        }
        
        if(event.renderedText === '!manastatus'){
          	getManaStatus();
        }
      }
    }
